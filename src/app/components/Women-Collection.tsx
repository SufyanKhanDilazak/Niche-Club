...
const CategoryBanner = memo<CategoryBannerProps>(({
  ...
  subtitle = "Discover our curated selection of premium womenswear designed for the modern woman",
  className = ""
}) => {
  return (
...
const WomensCollection = memo(() => {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const query = `*[_type == "product" && references(*[_type == "category" && title == "women-showcase"]._id) && defined(images[0].asset->url)] | order(_createdAt desc)[0...8] {
        _id,
        name,
        price,
        onSale,
