...
export function Header() {
  ...
  const logoSrc = isMounted && isThemeLoaded ? (isDarkMode ? '/logo.png' : '/logo1.png') : '/logo1.png';
      ...
      {isMounted && (
        <style jsx global>{`
          ...
          .cl-userButtonBox:hover {
            border: 1px solid var(--user-button-border-light); transform: scale(1.05) rotate(-5deg);
          ...
          .dark .cl-userButtonBox:hover {
            background-color: var(--user-button-bg-dark-hover); border: 1px solid var(--user-button-border-dark);
      ...
      <header className="fixed top-0 left-0 w-full z-50 border-b border-gray-200 dark:border-gray-800 bg-white dark:bg-black transition-colors duration-300 ease-in-out h-20 font-montserrat font-bold">
        <div className="container mx-auto max-w-7xl px-4 h-full flex items-center justify-between overflow-x-hidden">
          ...
          <div className="flex items-center lg:hidden">
            <Sheet open={isMenuSheetOpen} onOpenChange={setIsMenuSheetOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="bg-transparent hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-300 ease-in-out dark:text-white h-8 w-8 hover:scale-110 hover:-rotate-12">
                  <Menu className="h-[1.4rem] w-[1.4rem]" />
                </Button>
              </SheetTrigger>
              <SheetContent side="left" className="w-[300px] sm:w-[400px] bg-white dark:bg-gray-950 border-r border-gray-200 dark:border-gray-800 p-0 transition-all duration-300 ease-in-out">
                <div className="h-full flex flex-col">
                  ...
                  <div className="flex items-center justify-between p-6">
                    ...
                    <SheetClose asChild>
                      <button className="absolute right-4 top-4 rounded-sm opacity-70 transition-all duration-300 ease-in-out hover:opacity-100 hover:rotate-90 focus:outline-none">
                        <X className="h-4 w-4" />
                  ...
                  <nav className="flex flex-col space-y-6 px-6 pb-6 flex-1 overflow-auto">
                    <Link href="/" className={`relative py-2 transition-all duration-300 ease-in-out ${isActive('/') ? 'text-[var(--theme-primary)]' : 'text-black dark:text-white'} hover:text-[var(--theme-primary)] hover:translate-x-1`}>
                      HOME
                    ...
                    <div className="pt-4">
                      ...
                      <SignedOut>
                        <SignInButton mode="modal">
                          <Button variant="ghost" className="w-full justify-start pl-0 uppercase text-black dark:text-white hover:text-[var(--theme-primary)] transition-all duration-300 ease-in-out hover:scale-105">
          ...
          <div className="hidden lg:flex items-center space-x-8 flex-shrink-0">
            <Link href="/" className={`relative group uppercase transition-all duration-300 ease-in-out ${isActive('/') ? 'text-[var(--theme-primary)]' : 'text-black dark:text-white'} hover:text-[var(--theme-primary)] hover:-translate-y-0.5 hover:scale-105`}>
              HOME
          ...
          <div className="hidden lg:flex items-center space-x-8 ml-auto mr-4 flex-shrink-0">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href} className={`relative group uppercase transition-all duration-300 ease-in-out ${isActive(link.href) ? 'text-[var(--theme-primary)]' : 'text-black dark:text-white'} hover:text-[var(--theme-primary)] hover:-translate-y-0.5 hover:scale-105`}>
                {link.name}
                <span className={`absolute bottom-0 left-0 h-[1px] bg-gradient-to-r from-transparent via-[var(--theme-primary)] to-transparent shadow-[0_0_10px_2px_var(--theme-primary)] transition-all duration-300 ease-in-out ${isActive(link.href) ? 'w-full' : 'w-0 group-hover:w-full'}`} />
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center space-x-2 sm:space-x-3 ml-2 flex-shrink-0">
            <Button
              ...
              variant="outline"
              size="icon"
              className="rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 border border-gray-200 dark:border-gray-700 transition-all duration-300 ease-in-out bg-white dark:bg-gray-900 h-8 w-8 hover:scale-110 hover:-rotate-12"
              aria-label={isMounted && isThemeLoaded ? (isDarkMode ? 'Switch to light mode' : 'Switch to dark mode') : 'Toggle theme'}
            >
              {isMounted && isThemeLoaded ? (
                isDarkMode ? <Sun className="h-[1.3rem] w-[1.3rem] text-yellow-400 transition-all duration-300 ease-in-out" /> : <Moon className="h-[1.3rem] w-[1.3rem] transition-all duration-300 ease-in-out" />
              ) : (
            ...
            <Sheet open={isCartSheetOpen} onOpenChange={setIsCartSheetOpen}>
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[380px] bg-white dark:bg-gray-950 border-l border-gray-200 dark:border-gray-800 p-0 transition-all duration-300 ease-in-out">
                <div className="h-full flex flex-col">
                  <div className="absolute top-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[var(--theme-primary)] to-transparent shadow-[0_0_10px_2px_var(--theme-primary)] opacity-70" />
                  <div className="absolute bottom-0 left-0 w-full h-[1px] bg-gradient-to-r from-transparent via-[var(--theme-primary)] to-transparent shadow-[0_0_10px_2px_var(--theme-primary)] opacity-70" />
                  ...
                  <div className="flex-1 overflow-hidden flex flex-col">
                    {cartItems.length === 0 ? (
                      <div className="flex flex-col items-center justify-center flex-1 p-4 sm:p-6">
                        ...
                        <SheetClose asChild>
                          <Button className="bg-[var(--theme-primary)] hover:bg-[var(--theme-primary-hover)] text-white uppercase transition-all duration-300 ease-in-out hover:scale-105 text-sm">CONTINUE SHOPPING</Button>
                        </SheetClose>
                      </div>
                    ) : (
                      <>
            ...
            <div className="hidden lg:block flex-shrink-0">
              ...
              <SignedOut>
                <SignInButton mode="modal">
                  <Button variant="outline" size="icon" className="h-8 w-8 bg-white dark:bg-gray-900 hover:bg-gray-100 dark:hover:bg-gray-800 transition-all duration-300 ease-in-out border border-gray-200 dark:border-gray-700 rounded-full hover:scale-110 hover:-rotate-12">
