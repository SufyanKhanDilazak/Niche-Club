      ...
      <article className="relative">
        <div className="relative w-full aspect-[4/5] overflow-hidden bg-transparent rounded-sm">
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2 z-10">
            {product.newArrival && (
              <span className="relative bg-gradient-to-r from-emerald-500 via-green-500 to-teal-500 text-white px-3 py-1.5 text-xs font-bold shadow-xl rounded-full backdrop-blur-sm transition-all duration-300 hover:scale-105">
                NEW
              </span>
            )}
            {product.onSale && (
              <span className="relative bg-gradient-to-r from-red-500 via-pink-500 to-rose-500 text-white px-3 py-1.5 text-xs font-bold shadow-xl rounded-full backdrop-blur-sm transition-all duration-300 hover:scale-105">
                SALE
              </span>
            )}
          </div>
...
export default function MenTeesPage() {
  ...
  categories[]->{_id,title,slug}
      ...
      console.error('Error fetching men tees:', err);
      setError('Failed to load men tees');
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  return (
    <>
      ...
      <main className="min-h-screen font-montserrat bg-transparent sm:mt-28">
        ...
        <section className="text-center mb-16 font-montserrat bg-transparent">
            ...
            <div className="border border-blue-500 dark:border-[#a90068] bg-transparent backdrop-blur-md rounded-none sm:rounded-lg p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-6">
                {isLoading
                  ...
                  : products.length === 0
                  ? <div className="col-span-full text-gray-500">No men tees found</div>
                  : memoizedProducts}
              </div>
            </div>
          </div>
