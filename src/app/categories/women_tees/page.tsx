...
export default function WomenTeesPage() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      ...
      const query = `*[_type == "product" && references(*[_type == "category" && title == "women-tees"]._id)] | order(_createdAt desc) {
  _id,
  name,
  slug,
  price,
  ...
  categories[]->{_id, title, slug}
      ...
      console.error('Error fetching women tees:', err);
            ...
            <div className="border border-blue-500 dark:border-[#a90068] bg-transparent backdrop-blur-md rounded-none sm:rounded-lg p-4 sm:p-6">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5 sm:gap-6">
                {isLoading
                  ...
                  : products.length === 0
                  ? <div className="col-span-full text-gray-500">No women tees found</div>
                  : productCards}
              </div>
            </div>
          </div>
